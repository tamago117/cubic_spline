...
{
    std::vector<double> x{1.0, 2.0, 3.0, 5.0, 8.0, 10.0, 5.0, 0.0, 5.0};
    std::vector<double> y{1.0, 3.0, 7.0, 3.0, 6.0, 7.0, 13.0, 10.0, 8.0};

    CppCubicSpline2D cppCubicSpline2D(x, y);
    std::vector<std::vector<double>> output_path; //x ,y, yaw, curvature
    cppCubicSpline2D.calc_spline_course(output_path, 0.5);
    ...
    plt::scatter(x, y, 50, {{"color", "black"}});
    plt::plot(rx, ry);
